...
function grf_histo($anneedeb, $anneefin, $arrRes, $team, $ficgraf, $orig) {
  ...
  for($year = $anneedeb; $year <= $anneefin; $year++) {
    ...
    if (($nbnoTI + $nbavTI + $nbavOA) > $maxY) {
      $maxY = $nbnoTI + $nbavTI + $nbavOA;
    }
  }
	...
	if (strpos(phpversion(), "7") !== false) {//PHP7 > pChart2
		...
		for($year = $anneedeb; $year <= $anneefin; $year++) {
			...
			if($nbnoTI != 0){
				 $MyData->addPoints($nbnoTI, "Sans TI");
			} else {
			...
			if($nbavOA != 0){
				 $MyData->addPoints($nbavOA, "Avec lien externe vers PDF en OA");
			} else {
		...
		$avecOA = array("R"=>244,"G"=>120,"B"=>66,"Alpha"=>100);
		$MyData->setPalette("Avec lien externe vers PDF en OA",$avecOA);
